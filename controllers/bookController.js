    ...
    async.parallel({
        ...
        function (err, results){
            ...
            if (results.book_instance.length > 0) {
                ...
                BookModel.findByIdAndRemove(req.body.bookid, (err) => {
                    if (err) { return next(err); }
...
exports.book_update_get = function(req, res, next) {
    async.parallel({
            book : function (cb){
                BookModel.findById(req.params.id).populate('author').populate('genre').exec(cb);
            },
            authors : function (cb) {
                AuthorModel.find(cb);
            },
        ...
        function (err, results) {
            ...
            if(results.book == null){
                ...
                for(var i=0; i < results.genres.length; i++) {
                    if(results.book.genre.toString().indexOf(results.genres[i]._id.toString()) > -1){
                        results.genres[i].checked = true;
                    }
                }
...
exports.book_update_post = [
    ...
    (req, res , next) => {
        ...
        if(!errors.isEmpty()){
            ...
            // if no errors 
           BookModel.findByIdAndUpdate(req.params.id,book,{}, (err, results)=> {
                if(err) { return next(err); }
